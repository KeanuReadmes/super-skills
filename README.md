...
| [Super Skill Orchestrator](skills/super-skill.md) | Meta-skill that reads requirements and repo instructions, builds a verification inventory, walks work down an explicit G0–G4 granularity ladder with per-phase entry/exit criteria and a stable ID scheme, delegates atomization to `atomic-decomposer` under a strict delegation contract, assigns the best specialist skill and a risk-floored model tier to each slice, presents quality/cost options, and only proceeds to BMAD execution after approval (or via the documented fast path for trivial, pre-authorized work) |
