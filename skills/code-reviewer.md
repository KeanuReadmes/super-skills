...
- **Version & Environment Compatibility** — For every new language feature, framework API, library method, or service endpoint introduced in the diff, verify it is available at the **minimum declared version** in the relevant manifest (`package.json` `engines`, `go.mod`, `pyproject.toml` `requires-python`, `Cargo.toml` `rust-edition`, `.nvmrc`, `.tool-versions`, Helm `appVersion`, etc.). Cross-check against official changelogs, release notes, and compatibility matrices (e.g., MDN for browser APIs, caniuse.com, Node.js release schedule, Python version compatibility table). Additionally, for any service-to-service call, verify the API contract (endpoint, request/response shape, authentication scheme) is supported by the version deployed in **each environment** (dev, staging, production) — infer from environment manifests, Helm values files, `docker-compose` image tags, or CI environment variable declarations. Flag any feature, API, or syntax that is unavailable at the declared minimum or in any active deployment environment as `[MUST]`, stating the version where it was introduced and a concrete backport or compatible alternative.
...
3. **Dependency & version compatibility check** — Identify exact versions of all languages/frameworks/libraries in the manifests. Note new dependencies this PR introduces. For every new feature, API, or syntax in the diff, verify availability at the declared minimum version using official changelogs and compatibility matrices. Inspect environment manifests (Helm values, `docker-compose` image tags, CI env vars, `.tool-versions`) to determine the version deployed in each environment (dev, staging, prod); flag any feature or contract not satisfied in all environments as `[MUST]` with the introducing version and a compatible alternative.
...
[List every new language feature, library method, or service API introduced by this PR. Mark ✅ if available in all environments, or `[MUST]` if any environment is below the required version. Include the version it was introduced in and a concrete backport or compatible alternative for all `[MUST]` entries.]
...
- **Dependency upgrade** → Check changelog/migration guide for the exact version jump, verify deprecated-API usage, run `cargo audit` / `npm audit` / `pip-audit` / `trivy`, assess transitive-dependency blast radius. Run the version & environment compatibility check to confirm every consumer environment satisfies the new minimum version; flag any environment below the required minimum as `[MUST]`.
...
- **Version compatibility concerns** → For every new language feature, stdlib method, or service API in the diff: (1) identify the version it was introduced in from the official docs/changelog; (2) compare against the declared minimum in the manifest and against the version currently deployed in each environment; (3) flag `[MUST]` when any environment runs below the required version, stating the gap and a concrete alternative; (4) check browser/client-side APIs against the project's declared browserslist or target matrix (e.g., `caniuse.com`).
